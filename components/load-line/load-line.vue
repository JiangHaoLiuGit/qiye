			...
			<view class="circle" v-if="loadType=='circle'">
				<view class="third" :style="{background:circleInfo.background}">
					...
					<view class="mask" :style="{background:circleInfo.circleColor}">
						<text style="display: block;font-size: 36rpx;">{{loadPercent}}%</text>
						<text style="position: relative;top: 50rpx;font-size: 36rpx">在线率</text>
					</view>
				</view>
			</view>
			<view class="ball" v-if="loadType=='ball'" :style="{width:(ballSize_+25)+'px',height:(ballSize_+25)+'px'}">
				<view class="loading-ball" :style="{width:'340rpx',height:'340rpx',padding:ballInfo.ballPadding}">
					<view class="loading-wave" :style="{overflow:'hidden',width:ballSize_+'px',height:ballSize_+'px'}">
						...
						<view class="loading-text" :style="{lineHeight:+'22rpx',paddingTop:'94rpx'}">
